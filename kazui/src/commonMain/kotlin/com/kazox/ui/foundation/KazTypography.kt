...
/**
 * KazTypography defines the type scale for the design system.
 *
 ...
 * ```
 * val typography = kazTypography(fontFamily = myFont)
 * ```
 *
 * **2. Proportional scale (one number):**
 * ```
 * val large = kazTypography(fontFamily = myFont, scale = 1.15f)
 * val compact = kazTypography(fontFamily = myFont, scale = 0.85f)
 * ```
 *
 * **3. Individual size overrides:**
 * ```
 * val typography = kazTypography(
 *     fontFamily = myFont,
 *     h1Size = 48.sp,
 *     pSize = 18.sp,
 *     smallSize = 12.sp,
 ...
 */
@Immutable
public data class KazTypography(
...
public val LocalKazTypography: androidx.compose.runtime.ProvidableCompositionLocal<KazTypography> =
    staticCompositionLocalOf<KazTypography> {
        error(
            "No KazTypography provided. " +
                "Wrap your content in KazTheme { ... }",
        )
    }
...
/**
 * Creates a [KazTypography] with full customization.
 *
 ...
 * Text(
 *     text = "Important note",
 *     style = KazTheme.typography.p.emphasized(),
 * )
 * ```
 */
 ...
 *     style = KazTheme.typography.p.strong(),
 * )
 * ```
 */
