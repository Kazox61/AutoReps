...
/**
 * KazShapes defines the shape scale for the design system.
 *
 ...
 * ```
 * val shapes = kazShapes(radius = 10.dp)  // default
 * val sharp = kazShapes(radius = 4.dp)    // minimal rounding
 * val round = kazShapes(radius = 16.dp)   // very rounded
 * val square = kazShapes(radius = 0.dp)   // no rounding at all
 * ```
 *
 * **2. Presets:**
 * ```
 * val shapes = KazShapesPresets.sharp()    // 4dp — minimal
 * val shapes = KazShapesPresets.rounded()  // 16dp — soft
 * val shapes = KazShapesPresets.pill()     // 24dp — pill-like
 * val shapes = KazShapesPresets.square()   // 0dp — no rounding
 * ```
 *
 * **3. Full override:**
 * ```
...
public val LocalKazShapes: androidx.compose.runtime.ProvidableCompositionLocal<KazShapes> =
    staticCompositionLocalOf<KazShapes> {
        error(
            "No KazShapes provided. Wrap your content in KazTheme { ... }",
        )
    }

/**
 * Creates a [KazShapes] from a base radius value.
 *
 ...
 */
public fun kazShapes(radius: Dp = 10.dp): KazShapes {
    val r = radius.value
    return KazShapes(
 ...
 *
 * ```
 * val topRounded = (KazTheme.shapes.lg as CornerBasedShape).top()
 * ```
 */
 ...
 *
 * ```
 * KazTheme(shapes = KazShapesPresets.sharp()) { ... }
 * ```
 */
public object KazShapesPresets {
    /** No rounding. Brutalist, geometric. */
    public fun square(): KazShapes = kazShapes(radius = 0.dp)

    /** Minimal rounding. Clean, technical. */
    public fun sharp(): KazShapes = kazShapes(radius = 4.dp)

    /** Soft rounding. Friendly, modern. */
    public fun rounded(): KazShapes = kazShapes(radius = 16.dp)

    /** Heavy rounding. Pill-like, playful. */
    public fun pill(): KazShapes = kazShapes(radius = 24.dp)
