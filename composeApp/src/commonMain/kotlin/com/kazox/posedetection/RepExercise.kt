...
open class RepExercise(
    ...
    fun debug(pose: Pose): List<String> {
        return if (lastState == ExerciseState.END) {
            startPosition.angleConstraints.map { it.debug(pose) } +
                    startPosition.heightConstraints.map { it.debug(pose) }
        } else {
            endPosition.angleConstraints.map { it.debug(pose) } +
                    startPosition.heightConstraints.map { it.debug(pose) }
        }
    }
}
